...
SIGNAL = (inputs[LEFT_INPUT].getVoltage());
	...
	if (!std::isnan(mods[1])) outputs[RIGHT_OUTPUT].setVoltage(SIGNAL*pr+mods[1]);
		...
		if (rightExpander.module) {
			if ((rightExpander.module->model == modelMASTER) or (rightExpander.module->model == modelTRACK)) {
...
struct TRACKWidget : ModuleWidget {
	TRACKWidget(TRACK *module) {
setModule(module);
setPanel(APP->window->loadSvg(asset::plugin(pluginInstance, "res/TRACK.svg")));
	...
	addParam(createParam<Trimpot>(Vec(36, 47), module, TRACK::SEND1_PARAM));
	addParam(createParam<Trimpot>(Vec(36, 47+35), module, TRACK::SEND2_PARAM));
	addParam(createParam<Trimpot>(Vec(36, 47+70), module, TRACK::PAN_PARAM));
    	//addParam(createParam<RoundLargeBlackKnob>(Vec(27, 247), module, TRACK::GAIN_PARAM));
	addParam(createParam<FaderSliderWhite>(Vec(33.5, 190), module, TRACK::GAIN_PARAM));

     	addParam(createParam<LEDButton>(Vec(36, 208-55), module, TRACK::ON_PARAM));
	addChild(createLight<MediumLight<BlueLight>>(Vec(40.4, 212.4-55), module, TRACK::ON_LIGHT));

	addInput(createInput<PJ301MPort>(Vec(33, 321), module, TRACK::LEFT_INPUT));
	//addInput(createInput<PJ301MPort>(Vec(41, 321), module, TRACK::RIGHT_INPUT));

	for (int i = 0; i < 11; i++) {
		if (i<10) addChild(createLight<MediumLight<BlueLight>>(Vec(68, 159-i*11), module, TRACK::LEVEL_LIGHTS + i));
			else addChild(createLight<MediumLight<RedLight>>(Vec(68, 159-i*11), module, TRACK::LEVEL_LIGHTS + i));
	}
}
};


Model *modelTRACK = createModel<TRACK, TRACKWidget>("TRACK");
