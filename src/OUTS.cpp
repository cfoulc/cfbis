...
setModule(module);
setPanel(APP->window->loadSvg(asset::plugin(pluginInstance, "res/OUTS.svg")));
	...
	addParam(createParam<FaderSliderWhite>(Vec(33.5, 190), module, OUTS::GAIN_PARAM));

     	addParam(createParam<LEDButton>(Vec(36, 208-55), module, OUTS::ON_PARAM));
	addChild(createLight<MediumLight<BlueLight>>(Vec(40.4, 212.4-55), module, OUTS::ON_LIGHT));

	addOutput(createOutput<PJ301MPort>(Vec(9, 321-10), module, OUTS::SEND2_OUTPUT));
	addOutput(createOutput<PJ301MPort>(Vec(9, 321+15), module, OUTS::SEND1_OUTPUT));
	addOutput(createOutput<PJ301MPort>(Vec(56, 321-10), module, OUTS::LEFT_OUTPUT));
	addOutput(createOutput<PJ301MPort>(Vec(56, 321+15), module, OUTS::RIGHT_OUTPUT));

	for (int i = 0; i < 11; i++) {
		if (i<10) addChild(createLight<MediumLight<BlueLight>>(Vec(64, 159-i*11), module, OUTS::LEVEL_LIGHTS + i));
			else addChild(createLight<MediumLight<RedLight>>(Vec(64, 159-i*11), module, OUTS::LEVEL_LIGHTS + i));
	}
	for (int i = 0; i < 11; i++) {
		if (i<10) addChild(createLight<MediumLight<BlueLight>>(Vec(16, 159-i*11), module, OUTS::LEVEL2_LIGHTS + i));
			else addChild(createLight<MediumLight<RedLight>>(Vec(16, 159-i*11), module, OUTS::LEVEL2_LIGHTS + i));
	}
}
};
